...
<h1 class="header"><br>Orders</h1>
    ...
    <?php
        ...
        if ($result->num_rows > 0) {
            echo "<table>";
            echo "<tr>";
            echo "<th>ID</th><th>Customer ID</th><th>Order ID</th><th>Date Arrive</th><th>Product ID</th><th>Modify</th>";
            echo "</tr>";
            while ($row = $result->fetch_assoc()) {
                echo "<tr>";
                echo "<td>" . $row['id'] . "</td>";
                ...
                echo "<td>" . $row['prod_id'] . "</td>";
                echo "<td>";
                echo "<a href='completededit.php?id=" . $row['id'] . "'>Edit</a> | ";
                echo "<a href='completeddelete.php?id=" . $row['id'] . "' onclick='return confirm(\"Are you sure you want to delete this item?\")'>Delete</a>";
                echo "</td>";
                echo "</tr>";
            }
            echo "</table>";
    ...
    <h1 class="header"><br>Summary for Completed Delivery</h1>
    <?php
        include("config.php");
        $sql = " SELECT id, completed.customer_id, completed.order_id, date_arrive,QTY, price, customer_name, prod_name, completed.prod_id FROM completed, customer_info, product, orders WHERE completed.customer_id = customer_info.customer_id AND completed.order_id = orders.order_id AND completed.prod_id = product.prod_id";
        $result = $conn->query($sql);

        if ($result->num_rows > 0) {
            echo "<table>";
            echo "<tr>";
            echo "<th>ID</th><th>Customer ID</th><th>Order ID</th><th>Date arrive</th><th>Quantity</th><th>Price</th><th>Customer Name</th><th>Product Name</th><th>Product ID</th>";
            echo "</tr>";
            while ($row = $result->fetch_assoc()) {
                echo "<tr>";
                echo "<td>" . $row['id'] . "</td>";
